...
class HourLinePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // }
    /// * improved code by adding block on if statement because above code
    /// * does not have block on if statement
    if (showVerticalLine) {
      if (lineStyle == LineStyle.dashed) {
        var startY = 0.0;
        while (startY < size.height) {
...
/// {@template half_hour_line_painter}
/// Paints half hour lines.
/// {@endtemplate}
class HalfHourLinePainter extends CustomPainter {
  /// {@macro half_hour_line_painter}
  HalfHourLinePainter({
    required this.lineColor,
    required this.lineHeight,
    required this.offset,
